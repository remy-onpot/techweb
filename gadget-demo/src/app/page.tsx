...
import { BrandHero } from '@/components/home/BrandHero'; // Ensure this exists
...
export default async function HomePage() {
  ...
  const laptopProducts = allProducts.filter(p => p.category === 'laptop').slice(0, 7);
  const phoneProducts = allProducts.filter(p => p.category === 'phone').slice(0, 7);
  const gamingProducts = allProducts.filter(p => p.category === 'gaming').slice(0, 7);
  const audioProducts = allProducts.filter(p => p.category === 'audio').slice(0, 7);
  ...
  return (
    <div className="bg-[#F8FAFC] min-h-screen font-sans">
       ...
       <main className="pt-20 md:pt-28 space-y-0 md:space-y-2">
          ...
          {/* 1. BRAND HERO (Full Width Banner) - FIXED: Re-added here */}
          ...
          <div className="space-y-4 md:space-y-6 mb-8 md:mb-12">
            <CategoryRail category="laptop" products={laptopProducts} />
            <CategoryRail category="phone" products={phoneProducts} />
            <CategoryRail category="gaming" products={gamingProducts} />
            <CategoryRail category="audio" products={audioProducts} />
          </div>
